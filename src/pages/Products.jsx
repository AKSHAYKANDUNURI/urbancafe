...
import { FiEdit2, FiSearch, FiPlus, FiCamera, FiUpload } from 'react-icons/fi'
...
export default function Products() {
  ...
  const [form, setForm] = useState({ name: '', price: '', category: '', available: true, image: null })
  ...
  console.log('Products page - Raw products:', products)
...
if (form.image) {
  ...
  const base64 = await blobToBase64(compressed.data)
...
const handleSaveEdit = async () => {
  ...
  try {
    let imageData = editForm.image

    if (editForm.image instanceof File) {
      const compressed = await compressImage(editForm.image)
      const base64 = await blobToBase64(compressed.data)
  ...
  const handleCameraCapture = async (isEdit = false) => {
    try {
        ...
        <div className="card-glow">
          ...
          <div className="space-y-3 relative z-10">
            ...
            <div className="space-y-2">
              <label className="block text-xs uppercase tracking-[0.15em] text-stone-500 font-body">Product Image</label>
              <div className="flex gap-2">
                <label className="btn-secondary flex-1 flex items-center justify-center gap-2 py-2 cursor-pointer">
                  <FiUpload /> Upload
                  <input
                    type="file"
                    accept="image/*"
                ...
                <tbody>
                          ...
                          <td className="py-3 pr-4" colSpan={4}>
                            <div className="space-y-3">
                              <div className="grid grid-cols-2 gap-3">
                                <input className="input-field py-1.5 text-sm" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} placeholder="Name" />
                                <input type="number" className="input-field py-1.5 text-sm w-24" value={editForm.price ?? ''} onChange={(e) => setEditForm({ ...editForm, price: e.target.value })} placeholder="Price" />
                              </div>
                              ...
                              <div className="space-y-2">
                                <label className="block text-xs uppercase tracking-[0.15em] text-stone-500 font-body">Product Image</label>
                                <div className="flex gap-2">
                                  <label className="btn-secondary flex-1 flex items-center justify-center gap-2 py-2 cursor-pointer text-sm">
                                    <FiUpload /> Upload
                                    <input
                                      type="file"
                                      accept="image/*"
                  ...
                  {filtered.length === 0 && (
                    ...
                    <tr>
                      ...
                      <td colSpan={4} className="py-8 text-center text-slate-500">No products found</td>
